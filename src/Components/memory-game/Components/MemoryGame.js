...
class MemoryGame extends Component {
    ...
    handleClick = (imageID) => {
      ...
      if (!(foundPairs.includes(images[imageID]) || selectedImages.includes(images[imageID]))){
        ...
        //if it's is the second image of the guessed pair then check if both images are equal and reset selected images
          ...
          this.setState((prevState) => ({selectedImages: [...prevState.selectedImages,images[imageID]]}));
      }
    }
    ...
    updateBoard = () =>{
      ...
      urls = urls.map((element, index) => { 
        new Image().src =  "pictures/memory-game/"+imageType+"/"+element+".jpg";
        return {key: index, url: "pictures/memory-game/"+imageType+"/"+element+".jpg"}
      })
      this.setState({images: urls, boardSize: boardSize, foundPairs: [], selectedImages: []})
    }
    ...
    componentDidUpdate(prevProps){
      ...
      if (this.state.selectedImages.length===2){
          const { selectedImages, foundPairs} = this.state;
    ...
    render(){
      ...
      return <Board images={imagesComp} foundPairs={this.state.foundPairs} boardSize={this.state.boardSize}/>
    }
